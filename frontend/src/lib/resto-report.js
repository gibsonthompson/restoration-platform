...
//
// Used by routes/resto.js. generateReportPdf(graph, downloadImage) takes an
// injected async image downloader so it is testable without storage.
// ============================================================================
...
async function generateReportPdf(graph, downloadImage) {
  ...
  const h1 = (t) => { ensure(30); doc.moveDown(0.5).fillColor(ORANGE).fontSize(15).font('Helvetica-Bold').text(t); doc.moveDown(0.2).fillColor(DARK).font('Helvetica'); };
